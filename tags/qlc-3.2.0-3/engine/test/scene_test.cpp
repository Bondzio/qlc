...
void Scene_Test::writeValues()
{
    ...
    uni.zeroIntensityChannels();
    s1->writeValues(&uni, Fixture::invalidId(), QLCChannel::Intensity, 1.0);
    QCOMPARE(uni.preGMValues()[0], char(0));

    // Request non-participant fixture channels to be written
    uni.zeroIntensityChannels();
    s1->writeValues(&uni, fxi->id() + 1, QLCChannel::Intensity, 1.0);
    QCOMPARE(uni.preGMValues()[0], char(0));

    // Both channels are written
    for (qreal i = 0; i <= 1.0; i += 0.001)
    {
        uni.zeroIntensityChannels();
        s1->writeValues(&uni, Fixture::invalidId(), QLCChannel::NoGroup, i);
        QCOMPARE(uni.preGMValues()[0], char(floor((qreal(UCHAR_MAX) * i) + 0.5)));
        QCOMPARE(uni.preGMValues()[5], char(floor((qreal(UCHAR_MAX) * i) + 0.5)));
    }

    // 150% should be clamped to 100%
    uni.zeroIntensityChannels();
    s1->writeValues(&uni, Fixture::invalidId(), QLCChannel::NoGroup, 1.5);
    QCOMPARE(uni.preGMValues()[0], char(UCHAR_MAX));
    QCOMPARE(uni.preGMValues()[5], char(UCHAR_MAX));

    // Reset the LTP value at channel 0
    uni.reset(0, 1);

    // Negative percentage should be clamped to 0%
    uni.zeroIntensityChannels();
    s1->writeValues(&uni, Fixture::invalidId(), QLCChannel::NoGroup, -0.5);
    QCOMPARE(uni.preGMValues()[0], char(0));
    QCOMPARE(uni.preGMValues()[5], char(0));

    // Only the HTP channel should be written
    for (qreal i = 0; i <= 1.0; i += 0.001)
    {
        uni.zeroIntensityChannels();
        s1->writeValues(&uni, fxi->id(), QLCChannel::Intensity, i);
        QCOMPARE(uni.preGMValues()[0], char(0));
        QCOMPARE(uni.preGMValues()[5], char(floor((qreal(UCHAR_MAX) * i) + 0.5)));
    }
