...
/*
 * CHANGES:
 * 25.11.2006: Removed .owner from usbdmx_driver struct because it
 *   prevents compiling the driver with recent kernel 2.6 versions.
                        ...
                        const struct usb_device_id *id)
{
  ...
  info("%s now attached to usbdmx%d (MINOR = %d)", 
       device_information[type].name, 
       interface->minor - MINOR_ID, 
       interface->minor);
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
  return dmx;
#else
